...
/// this, each rebuild re-projected the markdown and re-wrapped every turn in
/// the session, so the cost of one new answer grew with the whole history.
#[derive(Default)]
...
#[cfg(test)]
mod tests {
    use super::{HistoryDocument, TurnRenderCache};
    use super::{StyledText, VisualRow};
    ...
    /// Wrapping may relocate characters but must never invent or lose them,
    /// and no row may overflow the viewport it was wrapped for.
    #[test]
